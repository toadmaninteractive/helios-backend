...
apply_modifier({Param, ModType}, Value) ->
    ...
    case ModType of
        ...
        ?mod_coalesce-> <<"COALESCE(", Param1/binary, ", ", Value1/binary, ")">>
    end.
...
-spec mk_query_filter_ex(OrderList, Offset, Limit) -> Result when
    OrderList :: [OrderItem],
    OrderItem :: {OrderBy, OrderDir} | {OrderBy, OrderDir, NullsFirst},
    OrderBy :: binary(),
    OrderDir :: binary(),
...
mk_query_filter_ex(OrderList, Offset, Limit) ->
    FmtNulls = fun
        (true) -> <<"NULLS FIRST">>;
        (false) -> <<"NULLS LAST">>;
        (undefined) -> <<>>
    end,
    OrderParts = [case OrderItem of
        {OrderBy, OrderDir} -> iolist_to_binary(io_lib:format("~s ~s", [OrderBy, order_dir(OrderDir)]));
        {OrderBy, OrderDir, NullsFirst} -> iolist_to_binary(io_lib:format("~s ~s ~s", [OrderBy, order_dir(OrderDir), FmtNulls(NullsFirst)]))
    end || OrderItem <- OrderList],
    OrderPartsCombined = util_binary:join(OrderParts, <<", ">>),
    Ordering = ?yesno(OrderParts =/= [], <<"ORDER BY ", OrderPartsCombined/binary>>, <<>>),
    OffsetPart = ?yesno(is_integer(Offset) andalso Offset >= 0, io_lib:format("OFFSET ~B", [Offset]), <<>>),
    LimitPart = ?yesno(is_integer(Limit) andalso Limit > 0, io_lib:format("LIMIT ~B", [Limit]), <<>>),
    Str = io_lib:format("~s ~s ~s", [Ordering, OffsetPart, LimitPart]),
    util_binary:trim(iolist_to_binary(Str)).
...
order_dir(Dir) when is_integer(Dir), Dir < 0 -> <<"DESC">>;
order_dir(Dir) when is_integer(Dir), Dir >= 0 -> <<"ASC">>;
order_dir(Dir) when is_atom(Dir) -> order_dir(atom_to_binary(Dir, utf8));
order_dir(Dir) when is_list(Dir) -> order_dir(iolist_to_binary(Dir));
order_dir(Dir) when is_binary(Dir) ->
    case util_binary:to_lower(Dir) of
        <<"asc">> -> <<"ASC">>;
        <<"desc">> -> <<"DESC">>;
        _ -> <<"ASC">>
    end.
