...
-spec get_one_by_api_key(ApiKey :: binary()) ->
    {'ok', jsx:json_term()} | {'error', Reason :: atom()}.

get_one_by_api_key(ApiKey) ->
    Query = <<
        "SELECT ", (common_entity_fields())/binary,
        "FROM personnel AS per ",
        (common_joins())/binary,
        "WHERE TRIM(per.api_key) = TRIM($1) "
    >>,
    case db_query:select_one(Query, [ApiKey]) of
        {ok, Account} -> {ok, Account};
        {error, Reason} -> {error, Reason};
        undefined -> {error, ?err_not_exists}
    end.
...
-spec get_consumer_ids(Guid, Platform) -> Result when
    Guid :: binary(),
    Platform :: web_protocol:platform(),
    Result :: {'ok', [non_neg_integer()]} | {'error', Reason :: atom()}.

get_consumer_ids(Guid, Platform) ->
    Query = <<
        "SELECT DISTINCT per.id ",
        "FROM personnel AS per ",
        "LEFT OUTER JOIN game_branches AS gbr ON (gbr.game_id = $1 AND gbr.platform = $2) ",
        "WHERE gbr.id IS NOT NULL AND is_game_accessible_by_personnel($1, per.id) "
    >>,
    Params = [Guid, web_protocol:platform_to_json(Platform)],
    case db_query:select(Query, Params) of
        {ok, Items} -> {ok, [PersonnelId || #{<<"id">> := PersonnelId} <- Items]};
        {error, Reason} -> {error, Reason}
    end.
...
profile(PersonnelId) ->
    Query = <<
        "SELECT ",
            (common_entity_fields())/binary, ", ",
            "per.api_key AS api_key ",
        "FROM personnel AS per ",
        (common_joins())/binary,
        "WHERE per.id = $1 "
    >>,
    case db_query:select_one(Query, [PersonnelId]) of
        {ok, Account} ->
            {ok, IsGameManager} = is_game_manager(PersonnelId),
            {ok, Account#{<<"is_game_manager">> => IsGameManager}};
        {error, Reason} -> {error, Reason};
        undefined -> {error, ?err_not_exists}
    end.
...
-spec is_game_manager(PersonnelId) -> Result when
    PersonnelId :: non_neg_integer(),
    Result :: {'ok', boolean()} | {'error', Reason :: atom()}.

is_game_manager(PersonnelId) ->
    Query = <<"SELECT is_game_manager($1) AS result">>,
    case db_query:select_one(Query, [PersonnelId]) of
...
-spec is_game_manager(Guid, PersonnelId) -> Result when
    Guid :: binary(),
    PersonnelId :: non_neg_integer(),
    Result :: {'ok', boolean()} | {'error', Reason :: atom()}.

is_game_manager(Guid, PersonnelId) ->
    Query = <<"SELECT is_game_manager($1, $2) AS result">>,
...
common_entity_fields() ->
    ...
    <<
        ...
        "is_game_manager(per.id) AS is_game_manager, ",
