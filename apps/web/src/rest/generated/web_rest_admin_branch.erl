...
get_game_branch_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

handle_get(Req) ->
    try
        Id = igor_http:parse_value(cowboy_req:binding(id, Req), long),
        {ResponseContent, Req1} = web_rest_callback_admin_branch:get_game_branch(Id, Req),
        Body = jsx:encode(web_protocol:game_branch_to_json(ResponseContent)),
