...
get_game_item_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

handle_get(Req) ->
    try
        Guid = cowboy_req:binding(guid, Req),
        {ResponseContent, Req1} = web_rest_callback_game:get_game_item(Guid, Req),
        Body = jsx:encode(web_protocol:game_item_to_json(ResponseContent)),
