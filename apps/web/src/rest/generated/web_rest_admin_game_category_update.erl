...
-module(web_rest_admin_game_category_update).
...
update_game_category_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

handle_put(Req) ->
    try
        {ok, RequestBody, Req1} = cowboy_req:read_body(Req),
        RequestContent = web_protocol:game_category_update_request_from_json(jsx:decode(RequestBody, [return_maps])),
        Id = igor_http:parse_value(cowboy_req:binding(id, Req1), long),
        Rev = igor_http:parse_value(cowboy_req:binding(rev, Req1), int),
        {ResponseContent, Req2} = web_rest_callback_admin_game_category:update_game_category(RequestContent, Id, Rev, Req1),
