...
-module(web_rest_admin_game_category_manage).
...
-export([
    init/2,
    assign_game_category_403/1,
    assign_game_category_404/1,
    assign_game_category_500/1,
    unassign_game_category_403/1,
    unassign_game_category_404/1,
    unassign_game_category_500/1
]).
...
assign_game_category_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

assign_game_category_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

assign_game_category_500(ResponseContent500) ->
...
handle_put(Req) ->
    try
        ...
        CategoryId = igor_http:parse_value(cowboy_req:binding(category_id, Req1), long),
        {ResponseContent, Req2} = web_rest_callback_admin_game:assign_game_category(RequestContent, Guid, CategoryId, Req1),
...
unassign_game_category_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

unassign_game_category_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

unassign_game_category_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_delete(Req) ->
    try
        Guid = cowboy_req:binding(guid, Req),
        CategoryId = igor_http:parse_value(cowboy_req:binding(category_id, Req), long),
        {ResponseContent, Req1} = web_rest_callback_admin_game:unassign_game_category(Guid, CategoryId, Req),
        Body = jsx:encode(web_protocol:generic_response_to_json(ResponseContent)),
        ResponseHeaders = #{<<"Content-Type">> => <<"application/json; charset=utf-8">>},
