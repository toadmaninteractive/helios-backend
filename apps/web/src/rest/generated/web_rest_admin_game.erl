...
get_game_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

handle_get(Req) ->
    try
        Guid = cowboy_req:binding(guid, Req),
        {ResponseContent, Req1} = web_rest_callback_admin_game:get_game(Guid, Req),
        Body = jsx:encode(web_protocol:game_to_json(ResponseContent)),
