...
-module(web_rest_admin_game_update).
...
update_game_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

handle_put(Req) ->
    try
        {ok, RequestBody, Req1} = cowboy_req:read_body(Req),
        RequestContent = web_protocol:game_update_request_from_json(jsx:decode(RequestBody, [return_maps])),
        Guid = cowboy_req:binding(guid, Req1),
        Rev = igor_http:parse_value(cowboy_req:binding(rev, Req1), int),
        {ResponseContent, Req2} = web_rest_callback_admin_game:update_game(RequestContent, Guid, Rev, Req1),
        Body = jsx:encode(web_protocol:game_update_response_to_json(ResponseContent)),
