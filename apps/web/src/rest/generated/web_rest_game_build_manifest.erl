...
-module(web_rest_game_build_manifest).
...
-export([
    init/2,
    get_game_build_manifest_403/1,
    get_game_build_manifest_404/1,
    get_game_build_manifest_500/1
]).
...
get_game_build_manifest_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

get_game_build_manifest_404(ResponseContent404) ->
    throw(#{status_code => 404, response => ResponseContent404}).

get_game_build_manifest_500(ResponseContent500) ->
    throw(#{status_code => 500, response => ResponseContent500}).

handle_get(Req) ->
    try
        Guid = cowboy_req:binding(guid, Req),
        BuildRev = cowboy_req:binding(build_rev, Req),
        {ResponseContent, Req1} = web_rest_callback_game:get_game_build_manifest(Guid, BuildRev, Req),
