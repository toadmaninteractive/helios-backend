...
get_games_403(ResponseContent403) ->
...
handle_get(Req) ->
    try
        ...
        OrderBy = igor_http:parse_query(<<"order_by">>, Qs, {custom, fun web_protocol:game_order_by_from_string/1}),
        ...
        {ResponseContent, Req1} = web_rest_callback_admin_game:get_games(Needle, OrderBy, OrderDir, Offset, Limit, Req),
        Body = jsx:encode(web_protocol:collection_slice_to_json(ResponseContent, {custom, fun web_protocol:game_to_json/1})),
...
create_game_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

handle_post(Req) ->
    try
        {ok, RequestBody, Req1} = cowboy_req:read_body(Req),
        RequestContent = web_protocol:game_create_request_from_json(jsx:decode(RequestBody, [return_maps])),
        {ResponseContent, Req2} = web_rest_callback_admin_game:create_game(RequestContent, Req1),
        Body = jsx:encode(web_protocol:game_create_response_to_json(ResponseContent)),
        ResponseHeaders = #{<<"Content-Type">> => <<"application/json; charset=utf-8">>},
