...
-module(web_rest_auth_personnel_profile).
...
-export([
    init/2,
    get_my_personnel_profile_403/1
]).
...
get_my_personnel_profile_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

handle_get(Req) ->
    try
        {ResponseContent, Req1} = web_rest_callback_personnel:get_my_personnel_profile(Req),
        Body = jsx:encode(web_protocol:personnel_account_profile_to_json(ResponseContent)),
