...
get_client_account_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

handle_get(Req) ->
    try
        Id = igor_http:parse_value(cowboy_req:binding(id, Req), long),
        {ResponseContent, Req1} = web_rest_callback_admin_client:get_client_account(Id, Req),
        Body = jsx:encode(web_protocol:client_account_to_json(ResponseContent)),
