...
-module(web_rest_admin_game_category).
...
-export([
    init/2,
    get_game_category_403/1,
    delete_game_category_403/1
]).
...
get_game_category_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

handle_get(Req) ->
    try
        Id = igor_http:parse_value(cowboy_req:binding(id, Req), long),
        {ResponseContent, Req1} = web_rest_callback_admin_game_category:get_game_category(Id, Req),
        Body = jsx:encode(web_protocol:game_category_to_json(ResponseContent)),
...
delete_game_category_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

handle_delete(Req) ->
    try
        Id = igor_http:parse_value(cowboy_req:binding(id, Req), long),
        {ResponseContent, Req1} = web_rest_callback_admin_game_category:delete_game_category(Id, Req),
