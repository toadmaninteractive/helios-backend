...
-module(web_rest_admin_build_file).
...
delete_build_file_403(ResponseContent403) ->
    throw(#{status_code => 403, response => ResponseContent403}).

handle_delete(Req) ->
    try
        Id = igor_http:parse_value(cowboy_req:binding(id, Req), long),
        FileId = igor_http:parse_value(cowboy_req:binding(file_id, Req), long),
        {ResponseContent, Req1} = web_rest_callback_admin_build_file:delete_build_file(Id, FileId, Req),
