...
-export([
    ...
    regenerate_personnel_api_key/2
]).
...
get_my_personnel_profile(#{?m_session := #session{key = {?actor_personnel, _}, user_id = UserId}} = Req) ->
    ...
    {web_protocol:personnel_account_profile_from_json(PersonnelAccountProfile), Req};
get_my_personnel_profile(_Req) ->
    web_rest_auth_personnel_profile:get_my_personnel_profile_403(#forbidden_error{}).

-spec regenerate_personnel_api_key(Request, Req) -> Response when
    Request :: web_protocol:empty(),
    Req :: cowboy_req:req(),
    Response :: {web_protocol:personnel_api_key_update_response(), cowboy_req:req()}.

regenerate_personnel_api_key(#empty{}, #{?m_session := #session{key = {?actor_personnel, _}, user_id = UserId}} = Req) ->
    ApiKey = <<"HLS-P-", (web_util:uuid64())/binary>>,
    ok = db_if_personnel:update(UserId, #{<<"api_key">> => ApiKey}),
    {#personnel_api_key_update_response{api_key = ApiKey}, Req};
regenerate_personnel_api_key(_Empty, _Req) ->
    web_rest_auth_personnel_api_key:regenerate_personnel_api_key_403(#forbidden_error{}).
